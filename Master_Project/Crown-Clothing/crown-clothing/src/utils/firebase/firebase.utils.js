...
import {
getFirestore,
doc,getDoc,setDoc,collection,writeBatch,query,getDocs
...
const firebaseConfig = {
  ...
  googleProvider.setCustomParameters({
   ...
   export const getCategoriesAndDoccuments = async ()=> {
    const collectionRef = collection(db,'categories')
    const q = query(collectionRef)

    const querySnapshot = await getDocs(q)

    const categoryMap = querySnapshot.docs.reduce((acc,docSnapsshot)=>{
      const {title,items} = docSnapsshot.data()
